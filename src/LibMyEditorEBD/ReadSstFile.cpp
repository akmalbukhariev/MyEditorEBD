...
bool ReadSstFile::read(EbdFileInfo& ebdFile, bool readThumbnail)
{
	...
	sstHeader = byteToStruct(ssHeaderData, sstHeader);
	...
	for (int i = 0; i < dCount; i += 3)
	{
		...
		else
		{
			...
			lPoint.realPoint = lPoint.calcDiffXY();
	...
	ebdFile.ebdHeader.DesignName = QString(sstHeader.Name).trimmed();
	ebdFile.ebdHeader.FileNameLength = QFileInfo(_fileName).baseName().length();
	ebdFile.ebdHeader.FileName = QFileInfo(_fileName).baseName();
